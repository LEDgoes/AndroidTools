...
public class DirectDrive extends Object {
	...
	protected static class driver extends AsyncTask<Void, Integer, Integer> {
	    ...
	    protected void onPreExecute() {
	        for (int i = 1; i < maxMsgs; i++)
	        	MyApp.messages[i] = "";
	        makeRMessageString(MyApp.messages[0]);
	        makeGMessageString(MyApp.messages[0]);
	        // Start a thread
	    }
	...
	//	protected Result doInBackground(Params... arg0) {
	    ...
	    protected void onPostExecute(Integer result) {
	        // Not sure just what to put here yet...
			//Toast.makeText(context, text, duration)
			Log.d("LEDgoes", "Message done");
	    }
	        ...
	        while (true)  // mind you, this is the implementation coming straight from the firmware ;)
	        {
	            ...
	            if (counter >= bufLen)          // If we've reached the end of the message,
	            {
	                ...
	                for (int i = maxMsgs - 1; i >= 0; i--)
	                {
	                    ...
	                    for (int j = 0; j < messages[i].length(); j++)  // Make an empty string equal in length to the current message
	                    	emptyString += ' ';
	    ...
	    public void writeString(int offset)
	    {
	        ...
	        MyApp.mLogService.write(eom);      // write the chip we want to write to over serial
	        MyApp.mLogService.write(rPart);
	        //Debug.Print(rBuf[counter].ToString());
	        // Write green portion
	        eom[0] |= 0x40;                    // green chips are looking for a message ID of 0xC0 to 0xFF
	        MyApp.mLogService.write(eom);      // write the chip we want to write to over serial
	        MyApp.mLogService.write(gPart);
